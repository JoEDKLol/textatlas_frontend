...
const Main = (props:any) => { 
  ...
  async function searchCommunityDetailUser(){
    ...
    const retObj = await transaction("get", "community/communitydetailusersearch", obj, "", false, true, screenShow, errorShow);
    if(retObj.sendObj.success === "y"){
      // setCommunity(retObj.sendObj.resObj);
      if(retObj.sendObj.resObj.hashtags.length > 0){
        setHashTagsArr(retObj.sendObj.resObj.hashtags);
      }
      setTextLength(retObj.sendObj.resObj.contents.replace(/<[^>]*>/g, '').length+1);
      setTitle(retObj.sendObj.resObj.title);
      setValue(retObj.sendObj.resObj.contents);
      // quillOnChangeHandler(retObj.sendObj.resObj.contents, null, null, QuillEditorScreen);
    }
  }
  ...
  async function saveWriting(){
    ...
    //savecommunitywriting

    const retObj = await transactionAuth("post", "community/updatecommunitywriting", obj, "", false, true, screenShow, errorShow);
    if(retObj.sendObj.success === "y"){ 
