...
const Main = (props:any) => {
  ...
  function wordListOnclick(index:number, word:string){
    ...
    if(wordList[index].reworkynKR){
      setContent(wordList[index].reworkmeaningKR);
    }else{
      setContent(wordList[index].meaningKR);
    }
  }
  ...
  async function wordSave(){
    ...
    const obj = {
      ...
      reworkmeaningKR:content,
      reworkynKR:newWordYn,
    }
    const retObj = await transactionAuth("post", "administrator/administratorwordupdate", obj, "", false, true, screenShow, errorShow);
// console.log(retObj);
    if(retObj.sendObj.success === "y"){ 
      // console.log(retObj);
      searchWordList();
  ...
  async function wordListSave(){
    ...
    const retObj = await transactionAuth("post", "administrator/administratorwordlistsave", obj, "", false, true, screenShow, errorShow);
// console.log(retObj);
    if(retObj.sendObj.success === "y"){ 
      // console.log(retObj);
      // searchWordList();
                        ...
                        <div className="mt-1   px-2 py-1 text-xs rounded-md ">
                          {
                            ...
                            <div dangerouslySetInnerHTML={{ __html: elem.reworkmeaningKR }} className="text-[10px]" />
                            :elem.meaningKR
                          }
                        </div>
          ...
          <table className="border w-full text-xs">
            <thead className="  ">
              <tr>
                ...
                <th className=" border ">mearning</th>
              </tr>
            </thead>
            <tbody>
            {
              excelData.map((elem:any, index:any)=>{
                return(
                  <tr key={index} className="">
                    ...
                    <td className=" border p-1 ">{elem.reworkmeaning}</td>
                  </tr>
                )
              })
            }
